...
| 値はあるが Email Routing で未検証のアドレス（雛形の `you@example.com` のまま等）で `forward()` が失敗（**他の必須設定は揃っている**場合） | **バウンスはしない**。分類済み経路（LEAD/REVIEW/分類失敗）は Slack 通知に⚠️付きで出す（Slack 未設定なら隔離ボックスの「未配信」行に出す）。分類前の素通し経路（`SKIPPED*` / `PARSE-FAILED`）は Slack 警告 + 例外終了で再送に委ねる |
...
- **値はあるが未検証のアドレス（雛形の `you@example.com` のまま等）は、他の必須設定が揃っていればバウンスしない**: `forward()` は実行時に失敗するが、恒久バウンスは suppression list 入りのリスクがあるため使わず、Slack 通知（⚠️付き）と隔離ボックスの「未配信」行で可視化する。**Slack も設定していない構成では、気づく手段が隔離ボックスだけになる**ので、セットアップ時に必ず実際のテスト送信で転送が届くことを確認すること（上の「転送できないときに何が起きるか」の表を参照）
