...
3. **事前検収 — あなたのメールでの精度測定（Step 2 で実例を預かった場合は必ず行う）**:
   ...
   - 経路C を選んだ場合: **追加のファイルは作らない**。eval.mjs は本番と同じ `src/criteria.ts` をそのまま読む（Node が型注釈を剥がして直接 import する）。そのため **Node.js 22.18 以上が必要**（推奨 24 LTS。古いと「TypeScript を直接読めません」と表示して停止する）。旧手順で作った `src/criteria.mjs` が残っていると検収は停止するので削除すること —— **検収用の写しを作ってはいけない**。写しを直しても本番のバンドルには載らず、「検収で直したのに古い基準がデプロイされる」乖離が起きる
   - 外れた例があれば**基準そのもの**（`src/criteria.js` / `src/criteria.ts` = 本番が読むファイル。eval が追記先のパスを表示する）に類型を追記して再実行（このとき検収用の例を基準生成に転用したら、その例はもう検収に使えない）
