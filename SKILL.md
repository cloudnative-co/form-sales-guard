...
2. **実例をありったけ集めてもらう**: 「実際に届いた迷惑な営業メッセージと、本物の問い合わせ（リード）を、あるだけコピーして貼ってください。目安は営業5通以上・リード3通以上で、多いほど精度が上がります」。メールを探して集める作業には時間がかかるので急かさず、揃うのを待つ
...
3. **事前検収 — あなたのメールでの精度測定（Step 2 で実例を預かった場合は必ず行う）**:
   - プロジェクト内に `samples/` を作り、Step 2-3 で取り置いた**検収用**の実例を `lead_01.txt` / `spam_01.txt` の命名で保存（テンプレート同梱の `.gitignore` で除外されていることを確認）
   - `ANTHROPIC_API_KEY=<キー> node <このリポジトリ>/tools/eval.mjs <プロジェクトのパス>` を実行し、正解率・ラベル別内訳・**リード喪失方向の誤り（LEAD→SPAM）**を利用者に見せる
